...
BirdBot posts a random bird from a cached catalog fetched from `https://ornithophile.vercel.app/api/birds` and renders its info and image directly in Discord.
...
On first run, the bot downloads the bird catalog JSON and saves it to `data/birds.json`. Later runs reuse that file.
